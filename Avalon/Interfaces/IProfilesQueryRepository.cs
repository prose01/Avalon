...
namespace Avalon.Interfaces
{
    public interface IProfilesQueryRepository
    {
        ...
        Task<(int totalPages, IReadOnlyList<Profile> profiles)> GetProfilesWhoBookmarkedMe(CurrentUser currentUser, OrderByType orderByType, int skip, int limit);
        Task<(int totalPages, IReadOnlyList<Profile> profiles)> GetProfilesWhoLikesMe(CurrentUser currentUser, OrderByType orderByType, int skip, int limit);
